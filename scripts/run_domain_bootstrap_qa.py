...
POST_INGESTION_QA_QUERY_STRATEGY: dict[str, Any] = {
    ...
    "arity_and_variable_policy": [
        ...
        "For product, retailer, store, state, geography, or distribution questions, do not answer from a broad state predicate alone when retailer-scoped predicates also exist. Prefer direct item-retailer-state carriers such as product_sold_at/3, product_retailer_in_state/3, bulk_item_distributed_to/3, or equivalent; otherwise join sibling rows with the same item variable, such as sold_at_retailer(Item, Retailer) plus sold_in_state(Item, State).",
...
def parse_numbered_markdown_questions(text: str) -> list[dict[str, Any]]:
    ...
    for raw_line in str(text or "").splitlines():
        ...
        if not match:
            continue
        questions.append({"id": qid, "number": number, "phase": phase, "utterance": utterance})
    return questions
    ...
    *,
    ...
    return {
        ...
        "result": {
            ...
            "variables": ["SourceRow", "SourceRecordLabel", "SourceRecordTextAtom", "SourceRecordSection"],
            "rows": out_rows,
            "reasoning_basis": {
                "kind": "core-local",
                "note": (
                    "source-record row context companion followed returned source-row ids "
                    "to same-row label/text/section evidence"
                ),
...
def run_one_question(
    *,
    ...
    source_citation_support = _source_record_citation_text_companion(runtime, utterance=utterance)
    if source_citation_support:
        query_results.append(source_citation_support)
    section_list_count_support = _source_record_section_list_count_companion(runtime, utterance=utterance)
    if section_list_count_support:
        query_results.append(section_list_count_support)
...
def _source_record_relative_next_day_companion(
    ...
    *,
    ...
    if not out_rows:
        return None
    return {
        "query": "source_record_relative_next_day_event(SourceRow, EventText, AnchorDate, DerivedDate).",
        "result": {
            "predicate": "source_record_relative_next_day_event",
            "prolog_query": "source_record_relative_next_day_event(SourceRow, EventText, AnchorDate, DerivedDate).",
            "result_type": "table",
            "status": "success",
            "num_rows": len(out_rows),
            "variables": [
            ...
            "reasoning_basis": {
                "kind": "core-local",
                "note": (
                    "source-record relative-date companion derived next-day event dates from an admitted "
                    "source text row containing an explicit anchor date and next-day cue"
                ),
                "original_question": utterance,
            },
        },
...
def _source_record_elapsed_date_duration_companion(
    ...
    *,
    ...
    text = str(utterance or "").casefold()
    if not re.search(r"\b(?:elapsed|how many days|duration|between)\b", text):
        return None
    if not re.search(r"\b(?:date|day|days|deadline|issuance|issued|opened|closed|published|report)\b", text):
        return None
    ...
    return {
        "query": "source_record_elapsed_date_duration_support(StartDate, EndDate, ElapsedDays, Duration).",
        "result": {
            "status": "success",
            "predicate": "source_record_elapsed_date_duration_support",
            "prolog_query": "source_record_elapsed_date_duration_support(StartDate, EndDate, ElapsedDays, Duration).",
            "result_type": "table",
            "num_rows": len(support_rows),
            ...
            "reasoning_basis": {
                "kind": "core-local",
                "note": (
                    "query-only elapsed-date support matched question date roles to admitted "
                    "source-record or direct date facts and computed calendar-day differences; no durable "
                    "duration fact was written"
                ),
                "utterance": utterance,
            },
        },
        "derived_from_queries": [
            ...
            "source_record_text_atom(SourceRow, TextAtom).",
            "direct admitted predicates with date-valued arguments",
        ],
    }
...
def _source_record_distinct_field_count_companion(
    ...
    *,
    ...
    return {
        "query": "source_record_distinct_field_count_support(Field, DistinctCount, DistinctValues).",
        "result": {
            "status": "success",
            "predicate": "source_record_distinct_field_count_support",
            "prolog_query": "source_record_distinct_field_count_support(Field, DistinctCount, DistinctValues).",
            "result_type": "table",
            "num_rows": len(support_rows),
            "variables": list(support_rows[0].keys()),
            "rows": support_rows,
            "reasoning_basis": {
                "kind": "core-local",
                "note": (
                    "query-only source-record aggregate counted distinct admitted field/item values "
                    "for a count-distinct question; no durable aggregate fact was written"
                ),
                "utterance": utterance,
            },
        },
...
def _source_record_earliest_date_field_pair_companion(
    ...
    *,
    ...
    return {
        "query": "source_record_earliest_date_field_pair_support(DateField, PairedField, CanonicalDate, PairedValues).",
        "result": {
            "status": "success",
            "predicate": "source_record_earliest_date_field_pair_support",
            "prolog_query": (
                "source_record_earliest_date_field_pair_support"
                "(DateField, PairedField, CanonicalDate, PairedValues)."
            ),
            "result_type": "table",
            "num_rows": len(support_rows),
            "variables": list(support_rows[0].keys()),
            ...
            "reasoning_basis": {
                "kind": "core-local",
                "note": (
                    "query-only source-record aggregate selected the earliest admitted date field "
                    "and kept sibling row-field values paired by SourceRow"
                ),
                "utterance": utterance,
            },
        },
        "derived_from_queries": [
            ...
            "source_record_date_alias(SourceRow, DateValue, CanonicalDate).",
            "source_record_field(SourceRow, PairedField, PairedValue).",
        ],
    }
...
def _source_record_max_numeric_field_companion(
    ...
    *,
    ...
    return {
        "query": "source_record_max_numeric_field_support(Field, MaxValue, SourceRow, TypeOfBusiness).",
        "result": {
            "status": "success",
            "predicate": "source_record_max_numeric_field_support",
            "prolog_query": "source_record_max_numeric_field_support(Field, MaxValue, SourceRow, TypeOfBusiness).",
            "result_type": "table",
            "num_rows": len(support_rows),
            "variables": list(support_rows[0].keys()),
            "rows": support_rows,
            "reasoning_basis": {
                "kind": "core-local",
                "note": (
                    "query-only source-record aggregate normalized messy numeric field atoms, selected "
                    "the maximum, and kept sibling row fields joined by SourceRow"
                ),
                "utterance": utterance,
            },
        },
...
def _source_record_weather_observation_companion(
    ...
    *,
    ...
    if not support_rows:
        return None
    return {
        "query": "source_record_weather_observation_support(SourceRow, Location, ThunderstormStart, MaxWindGust, ThunderstormEnd, Rainfall).",
        "result": {
            "status": "success",
            "predicate": "source_record_weather_observation_support",
            "prolog_query": (
                "source_record_weather_observation_support"
                "(SourceRow, Location, ThunderstormStart, MaxWindGust, ThunderstormEnd, Rainfall)."
            ),
            "result_type": "table",
            "num_rows": len(support_rows),
            "variables": list(support_rows[0].keys()),
...
def _source_record_issued_product_chronology_companion(
    ...
    *,
    ...
    return {
        "query": "source_record_issued_product_chronology_support(Time, ProductType, SourceRow).",
        "result": {
            "status": "success",
            "predicate": "source_record_issued_product_chronology_support",
            "prolog_query": "source_record_issued_product_chronology_support(Time, ProductType, SourceRow).",
            "result_type": "table",
            "num_rows": len(support_rows),
            "variables": list(support_rows[0].keys()),
            "rows": support_rows,
            "reasoning_basis": {
                "kind": "core-local",
                "note": (
                    "query-only chronology companion extracted issued weather-product sequence "
                    "from admitted source_record_text_atom rows"
                ),
                "utterance": utterance,
            },
...
def _source_record_signatory_conflict_companion(
    ...
    *,
    ...
    return {
        "query": "source_record_body_signatory_support(Person, Roles, MetadataSignatories, MetadataConflict).",
        "result": {
            "status": "success",
            "predicate": "source_record_body_signatory_support",
            "prolog_query": "source_record_body_signatory_support(Person, Roles, MetadataSignatories, MetadataConflict).",
            "result_type": "table",
            "num_rows": len(support_rows),
            "variables": list(support_rows[0].keys()),
            "rows": support_rows,
            "reasoning_basis": {
                "kind": "core-local",
                "note": (
                    "query-only signatory companion cross-checked source metadata with body/signature-block "
                    "source-record rows and entity_role rows"
                ),
                "utterance": utterance,
            },
        },
        "derived_from_queries": [
            "source_metadata(Source, Type, Issuer, Date, Signatory).",
            "entity_role(Person, Role).",
            "source_record_row_context(SourceRow, Person, TextAtom, Section).",
        ],
    }
