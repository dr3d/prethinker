...
def _rule_guidance_context(*, target: int, rule_class: str, compact: bool) -> list[str]:
    core = [
        ...
        "A rule clause must have Head :- Body form. Do not submit a fact-shaped clause such as threshold(action, 4, 7). under operation='rule'.",
        ...
        "Every head variable must be bound by at least one body goal. If you cannot bind a head variable from existing_admitted_backbone, use a source-stated constant or emit no rule.",
        "For threshold or requirement records, bind the head subject from the record predicate body; do not put a fresh subject variable in the head unless the same variable appears in a body goal.",
        ...
        "Do not use derived_* predicates in the body unless an existing admitted backbone rule or current explicit composition dependency already provides that derived predicate. First-order rule lenses should use source/backbone predicates in their bodies.",
        "Avoid fake generality. Prefer one body-supported intermediate condition over a broad rule that cannot bind its subject safely.",
