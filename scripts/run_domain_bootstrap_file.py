...
OPERATIONAL_RECORD_STATUS_CONTEXT_V1 = [
    ...
    "Operational lifecycle palette construction rule: for permit, intake, docket, queue, sample, ticket, accession, or repair lifecycle sources, include the canonical lifecycle predicates when the source states those surfaces and no stricter domain-specific predicate already carries the same slots. Do not invent fixture-local predicate names for initial/current/final status, record aliases, receipt/logging layers, or supersession targets if the canonical lifecycle predicates can carry them.",
...
COMPILE_SURFACE_INVARIANT_CONTEXT_V1 = [
    ...
    "Compile surface invariant rule: repeated lifecycle/status source lines require parallel preservation. For each stated dated lifecycle/status line, emit at least one complete direct unit that keeps subject, lifecycle state/action, and date/turn joinable; do not satisfy the series with only supersession fragments, event labels, or status words.",
