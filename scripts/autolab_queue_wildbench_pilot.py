...
def build_job_markdown(
    *,
    ...
    lines = [
        ...
        "Keep the whole job under six minutes. If you are not holding a usable source packet by minute four, write a blocked report, validate it, update the summaries, and stop.",
        "",
        "## Goal",
        "",
        ...
        "",
        "For this pilot, prefer accessible static sources first: GitHub repositories with public markdown/PDF packets, arXiv papers with appendices, museum/archive pages, static city or board minutes, and non-bot-protected policy pages.",
        "Avoid federal search portals and bot-protected government discovery pages unless a direct static document URL is already available.",
        "",
        "If web/network access is unavailable, write a structured blocked report to the run directory and stop successfully. Do not invent sources.",
        "",
        "## Required Run Directory",
        "",
        f"Create `{run_dir}`.",
        "",
        "Before web search, create these files with `status: running` or an equivalent draft status:",
        "",
        f"- `{run_dir}/wildbench_pilot_summary.md`",
        f"- `{run_dir}/wildbench_pilot_summary.json`",
        "",
        "If access is blocked, immediately write:",
        "",
        f"- `{run_dir}/source_hunt_blocked.json`",
        "",
        "Then run validation, update the summaries, and stop. A blocked hunt should not create fake candidate directories.",
        "A timeout with only running summaries is a failed job. The useful artifact is either a real candidate or a valid blocked report.",
        "",
        "For each real source candidate, create a subdirectory:",
        "",
        *candidate_dirs,
        "",
    ...
    lines.extend(
        [
            ...
            "```",
            "",
            "## Blocked Source Hunt JSON",
            "",
            "Use this schema if no candidate can be produced because source access is blocked or unavailable:",
            "",
            "```json",
            "{",
            ...
            '  "notes": "short non-source-prose operational note"',
            "}",
            "```",
            "",
            "This blocked report is a successful research artifact only if it is written as JSON and passes validation. Do not bury blocked findings only in stdout.",
            "",
        ]
    )
    if not source_only:
