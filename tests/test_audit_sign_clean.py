...
def test_sign_clean_audit_allows_claims_when_only_forensic_free_text_routing_remains() -> None:
    ...
    assert report["summary"]["free_text_forensic_or_structural_count"] > 0
...
def test_sign_clean_audit_has_no_raw_utterance_or_legacy_vocabulary_blockers() -> None:
    ...
    assert "free_text_semantic_routing" not in blocker_classes
