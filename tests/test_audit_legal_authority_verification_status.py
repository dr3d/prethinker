...
def test_legal_authority_status_audits_manifest_fixtures() -> None:
    ...
    assert summary["matched_expected_fact_count"] == summary["expected_fact_count"] == 56
    assert summary["matched_forbidden_fact_count"] == 0
    assert summary["forbidden_fact_count"] == 16
    assert summary["citation_mentions"] == 10
    assert summary["false_verified"] == 0
    assert summary["verified_mentions"] == 1
    assert summary["blocked_mentions"] == 8
...
def test_legal_authority_status_markdown_names_false_verified_gate() -> None:
    ...
    assert "Expected facts: `56 / 56`" in markdown
    assert "Matched forbidden facts: `0 / 16`" in markdown
