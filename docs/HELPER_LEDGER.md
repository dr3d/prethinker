...
| `industrial_sensor_support` | 1 | yes | split, audit ongoing | transfer: 37 clean / 7 candidate | Event/timestamp/count, computed duration, vendor/model, batch-id, ticket, packet-id, data-loss status, and field-derived rows are mostly clean after recent cleanup; stated-duration and exact prose recognizers remain candidate. | Continue retiring candidate recognizers into generic source-record extraction. |
