...
| `industrial_sensor_support` | 1 | yes | split, audit ongoing | transfer: 35 clean / 9 candidate | Event/timestamp/count, computed duration, vendor/model, batch-id, ticket, and field-derived rows are mostly clean after recent cleanup; stated-duration and exact prose recognizers remain candidate. | Continue retiring candidate recognizers into generic source-record extraction. |
