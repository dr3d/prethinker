...
| `source_record_packet_metadata_support` | 3 | yes | split, audit ongoing | transfer class audit after grant reference repair: 171 clean / 26 candidate; cleanup audit: 17 identifier kinds / 10 content-note kinds | Generic identifiers, source-reference list rows, and filing-location metadata are clean when derived mechanically from source-record atoms/sections. The remaining candidate rows are content notes such as appeal funding source, appeal pending status, observer permission scope, pending packet item, role definitions, and transport departure. | Keep generic identifiers and source-reference metadata; quarantine embedded content notes until moved to domain helpers or retired as duplicates. |
