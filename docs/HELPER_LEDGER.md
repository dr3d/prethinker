...
| `item_description_detail_support` | 2 | yes | transfer-shown clean-helper over admitted `item_description/2` atoms | probate residual probe: 25 clean / 0 candidate; probate full replay: 62 clean / 0 candidate; identifier focused QA probe: 33 clean / 0 candidate with `2 / 2 / 0` score | Query-only bridge that derives display title and trailing year from admitted `item_description/2` atoms. This is a generic semantic-predicate completeness bridge, not a source text recognizer. Sibling proof on identifier-ledger artifacts shows the helper transfers when the canonical predicate exists. The remaining boundary is predicate availability and plain display normalization: some item-like rows, such as EX-U-1 in identifier ledger, are not admitted as `item_description/2`, and abbreviations/possessives are only lightly formatted. | Keep promoted narrowly over admitted `item_description/2`; do not use it to infer missing descriptions from source text. Future work should improve canonical predicate completeness or add a separate deterministic item-description ledger if enough fixtures need it. |
...
clean substrate only when `item_description/2` has already been admitted. Every
other helper is still one-fixture by usage count. That does not make them wrong,
but it means their scores must remain labeled as narrow, candidate, or awaiting
