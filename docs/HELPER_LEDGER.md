...
| `source_record_packet_metadata_support` | 3 | yes | split, audit ongoing | transfer class audit after grant reference repair: 171 clean / 26 candidate; probate targeted replay: 1150 clean / 0 candidate; probate full replay: 2668 clean / 0 candidate | Generic identifiers, source-reference list rows, filing-location metadata, official-prose standing notes, role-holder headings, custody-location fields, non-reproduced references, authoritative-source statements, loan-amendment effects, and non-revocable access-policy rows are clean when derived mechanically from source-record atoms/sections/fields. The remaining candidate rows are older content notes such as appeal funding source, appeal pending status, observer permission scope, pending packet item, role definitions, and transport departure. | Keep generic identifiers/source-reference/standing metadata; quarantine embedded content notes until moved to domain helpers or retired as duplicates. |
...
`9 / 0 / 0` with only clean-helper rows, while the full replay stayed at
`34 / 2 / 4` because QA/query generation did not always use those rows.
