...
| `industrial_sensor_support` | 1 | yes | split, audit ongoing | transfer: 39 clean / 5 candidate | Event/timestamp/count, computed duration, vendor/model, batch-id, ticket, packet-id, data-loss status, lab-sample status/return, and field-derived rows are mostly clean after recent cleanup; stated-duration and remaining exact prose recognizers remain candidate. | Continue retiring candidate recognizers into generic source-record extraction. |
