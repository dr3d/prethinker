...
IR fallback for no-query compliance intents move it to `30 / 2 / 8`. Row-level
churn remains, so the next work is selector/query discrimination over emitted
helper rows, not promotion.
