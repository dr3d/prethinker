...
| `industrial_sensor_support` | 1 | yes | split, audit ongoing | transfer: 40 clean / 4 candidate | Event/timestamp/count, computed duration, vendor/model, batch-id, ticket, packet-id, data-loss status, lab-sample status/return, system clock authority, and field-derived rows are mostly clean after recent cleanup. Stated-duration rows remain candidate; the root-cause packet-scope row also remains candidate because the ledger currently preserves only the weaker `tbd_root_cause_analysis_report` table row, not the source paragraph saying root cause is not assigned in this packet. | Continue retiring candidate recognizers into generic source-record extraction; treat the root-cause row as a source-acquisition gap unless the refusal paragraph becomes addressable. |
