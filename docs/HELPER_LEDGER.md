...
| `source_record_table_body_count_support` | 1 | yes | clean structural substrate; QA-routing pending | probate residual probe: 5 clean / 0 candidate; probate full replay: 10 clean / 0 candidate; sibling artifact audit: clinic 1 row, industrial 5 rows, school 8 rows, grant 7 rows; focused sibling QA: 8 / 0 / 0 but 0 natural helper invocations | Counts field-bearing `source_record_row(..., table_row, ...)` body rows while excluding header rows whose values are only column names. This is generic deterministic table addressability, and artifact-only transfer probes found plausible table-body counts on four sibling fixtures without false header/body positives. The natural QA planner still answered sibling count rows through stronger domain helpers, so this is not yet production QA transfer proof. | Add planner/query routing for explicit table body count intents, then rerun the sibling focused probe and require this helper to appear in QA companion summaries without reducing exactness before promoting it. |
