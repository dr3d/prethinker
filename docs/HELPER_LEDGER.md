...
| `grant_award_support` | 1 | yes | split, audit ongoing | transfer class audit after cleanup: 32 clean / 5 candidate; fresh QA replay: 1008 clean / 252 candidate | Award, cap, eligibility, field-recusal, appeal-window, committee-recusal vote-count, score-correction operational status, and appeal-pending status rows are mostly clean over admitted predicates plus generic source-record atoms/sections. A scar cleanup removed hard-coded `a_07` / `2026-05-22` assumptions from appeal review/funding extraction, but the helper remains one-fixture and still emits candidate appeal/procedure rows. Fresh replay scores `37 / 1 / 2` with zero writes. | Treat grant as candidate-helper evidence until sibling transfer proof or further source-record acquisition closes q039/q040 filing/reference gaps. |
...
Latest usage audit:
`tmp/helper_usage_audit_20260510/helper_usage_audit_after_grant_cleanup.md`

- QA JSON artifacts scanned: 514
