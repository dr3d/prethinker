...
| `clinic_recall_support` | 1 | yes | split, audit ongoing | transfer cold: 19 clean / 7 candidate; refreshed clinic: 19 clean / 10 candidate | Device/serial field rows plus generic manufacturer-liaison, verification-procedure, acronym-derived clinic abbreviations, and failure-rate atoms are clean when the needed source-record rows exist; non-initialism abbreviations such as `NBFH`, cabinet/seal/key, authority, and remaining procedure/clinic recognizers remain candidate. | Rewrite candidate recognizers over generic source-record fields/labels or quarantine them. |
