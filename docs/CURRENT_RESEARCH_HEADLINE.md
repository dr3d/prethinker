...
Current domain-pack claims require:

- closed profile registry with lens-scoped offered signatures;
- N>=3 same-condition compiles with support>=2;
...
Targeted replays and composed historical runs are mechanism evidence, not
transfer claims.
...
SEC Form 8-K skeleton pack
  ...
  transfer_003:
    ...
    source-only review `sec_t001_source_review_20260605` rejected all 10
...
6. Treat atom-library query grounding as the strict query path: planner payloads
   ...
   transfer_003 five-row atom-query smoke reached a favorable single-run
   `5/5` exact, but the first pre-registered variance cell is the honest
   picture: local Qwen temp-0 N=5 landed at `23/25` thesis-clean exact, with
   all exact rows typed-plan and redaction replay clean; Qwen temp `0.2` and
   ...
   This is query-planner evidence over typed atoms, not permission to use prose
   or fallback rescue.
