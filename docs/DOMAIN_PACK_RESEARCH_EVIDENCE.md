...
source-scope filing-id integrity guard, the current standing t001 cell is 3/13
with 4/0 forbidden emissions at support>=1/support>=2. The remaining blocker
is source-reviewed registrant-id aliasing, not prose leakage and not a safe
place for a company-specific reducer.
...
- SEC shows that a small closed skeleton pack can transfer parts of recurring
  ...
  manifest now lands at seed 13/13, transfer_001 3/13, transfer_002 10/12, and
  transfer_003 10/13 after source-only review of transfer_001 tightened the
  forbidden set and a typed source-scope filing-id guard removed wrong-key
  support. A later 2026-06-05 transfer_003 refresh landed at a favorable 13/13
...
rejected all ten inspected variants and tightened the current standing cell to
3/13 with 4/0 forbidden emissions at support>=1/support>=2. The safe
source-scope filing-id guard was promoted; the remaining registrant alias
emissions are a visible blocker, not a prompt-polish target. Retained artifacts:
...
The 2026-06-05 manifest run is currently red after importing the
source-only SEC transfer_001 review. The review rejected all ten inspected
candidate variants. A safe typed source-scope filing-id guard removed
fixture-id-as-filing-id support, while source-reviewed registrant aliases
remain as supported forbidden emissions at support>=1:

```text
overall support>=2: 105/141
SEC seed/transfer support>=2: 13/13, 3/13, 10/12, 10/13
...
forbidden emissions support>=1/support>=2: 4/0
status: fail
```
...
The latest generated 2026-06-05 status is intentionally red: the compile-fact
QA manifest and current compile-fact QA status fail only on
`sec_form_8k_skeleton_transfer_001:forbidden_emissions_ge_1:4`. The SEC seed
