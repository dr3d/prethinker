...
guidance, the current standing t001 cell is 11/13 with 0/0 forbidden emissions
at support>=1/support>=2 and 0 unexpected same-signature rows. The remaining
boundary is telephone and signatory support, not prose leakage and not
source-reviewed alias emission.
...
- SEC shows that a small closed skeleton pack can transfer parts of recurring
  ...
  manifest now lands at seed 13/13, transfer_001 11/13, transfer_002 10/12,
  ...
  without a company-specific reducer. A later 2026-06-05 transfer_003 refresh
  landed at a favorable 13/13 with 0 supported forbidden, but that is one point
  inside the observed repaired-schema band. SEC is a boundary-aware case study,
  not a pristine anchor.
...
| SEC | transfer_001 no-truncation contract probe | `C:\prethinker_tmp_archive\sec_t001_no_truncate_contract_20260606\sec-t001-no-truncate-full-r1-20260606` | Current standing transfer_001 probe: 11/13 support>=2, 0/0 forbidden emissions, 0 unexpected same-signature rows, clean atom/lens/value gates. Telephone and signatory remain unsupported boundaries. |
...
cell to 11/13 with 0/0 forbidden emissions at support>=1/support>=2 and 0
unexpected same-signature rows. Telephone and signatory support remain below
the claim threshold. Retained artifacts:
...
inspected candidate variants. The latest no-truncation probe recovers expected
registrant/exhibit rows without emitting source-reviewed aliases as supported
forbidden facts:

```text
overall support>=2: 113/141
SEC seed/transfer support>=2: 13/13, 11/13, 10/12, 10/13
