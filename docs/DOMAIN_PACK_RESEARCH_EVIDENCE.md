...
- SEC shows that a small closed skeleton pack can transfer across three unlike
  ...
  invalid omission rows without lifting support. SEC is a boundary-aware case
  study, not a pristine anchor.
...
- OSHA adds fourth-family corroboration from retained public fixtures: accident,
  injury, violation-count, penalty, item, and status rows transfer under clean
  governance, while wrapper/establishment and blank-value rows remain weaker.
...
the 12-command governance run passed. This is guard tightening, not a support
