...
source-scope filing-id integrity guard and adding legal-name/exhibit-treatment
contract guidance, the current standing t001 cell is 9/13 with 2/0 forbidden
emissions at support>=1/support>=2. The remaining blocker is source-reviewed
registrant-id aliasing, not prose leakage and not a safe place for a
company-specific reducer.
...
| SEC Form 8-K skeleton pack | One seed micro plus three unlike retained Form 8-K/8-K/A transfers | Closed SEC skeleton registry, local-Qwen N>=3/support>=2, integrity gates plus value-axis integrity | Historical pre-axis-repair cells: seed 13/13; transfers 13/13, 12/12, 12/12, all 0 supported forbidden. Axis-clean repair split `sec_filing_item/5`, `sec_filing_item_treatment/4`, and `sec_exhibit/5`; the current standing manifest is now seed 13/13, transfer_001 9/13, transfer_002 10/12, transfer_003 10/13 after importing source-only review, adding the typed source-scope filing-id guard, and adding legal-name/exhibit-treatment contract guidance. Fresh repaired transfer_003 roots still span 11-13/13 historically, but the current standing manifest is red because transfer_001 emits two source-reviewed registrant alias rows at support>=1. The seed current-mainline retest landed at 13/13 with 39/39 per-run exact and 0/7 supported forbidden, but also five singleton unexpected item-treatment rows. | Skeleton transfers unevenly; event-substance intentionally untested. Current caveat is transfer_001 source-reviewed alias emission, same-condition jitter/favorable-draw risk, wrapper/telephone recall, duplicate file-number support, and item-treatment recall under the repaired schema. The source-scope guard is safe; a company-specific alias reducer is not. | Use SEC as a boundary-aware skeleton case study; do not promote it to a pristine methods anchor or claim-bearing green manifest until the alias blocker closes cleanly |
...
- SEC shows that a small closed skeleton pack can transfer parts of recurring
  ...
  manifest now lands at seed 13/13, transfer_001 9/13, transfer_002 10/12, and
  transfer_003 10/13 after source-only review of transfer_001 tightened the
  forbidden set, a typed source-scope filing-id guard removed wrong-key
  support, and legal-name/exhibit-treatment contract guidance recovered
  support without closing the alias blocker. A later 2026-06-05 transfer_003
  refresh landed at a favorable 13/13
  with 0 supported forbidden, but that is one point inside the observed
  repaired-schema band. SEC is a boundary-aware case study, not a pristine
  anchor, and the current standing manifest is red until the source-reviewed
  registrant-alias blocker closes cleanly.
...
| SEC | transfer_001 legal-name/exhibit-treatment contract probe | `C:\prethinker_tmp_archive\sec_t001_contract_probe_20260606\sec-t001-exhibit-treatment-boundary-r1-20260606` | Current standing transfer_001 probe: 9/13 support>=2, 2 forbidden emissions at support>=1 and 0 at support>=2, clean atom/lens/value gates. The remaining blocker is source-reviewed `svc_now_inc` alias emission; the contract guidance is general, not a company-specific reducer. |
| SEC | transfer_001 source-only candidate review | `datasets/candidate_oracle_reviews/sec_t001_source_review_20260605_sec_exhibit_5`; `datasets/candidate_oracle_reviews/sec_t001_source_review_20260605_sec_registrant_4`; `datasets/candidate_oracle_reviews/sec_t001_source_review_20260605_sec_registrant_identifier_5` | Imported source-only review of ten candidate variants: 0 expected, 10 forbidden. This review rejects exhibit-treatment overstatements, registrant-id aliases, and fixture-id-as-filing-id rows. It is the basis for the current red manifest blocker. |
...
promoted, and a later legal-name/exhibit-treatment contract probe lifted the
current standing cell to 9/13, but 2/0 forbidden emissions at
support>=1/support>=2 remain from source-reviewed `svc_now_inc` alias rows.
Those aliases are a visible blocker, not a prompt-polish target. Retained
artifacts:
...
The 2026-06-06 manifest run is currently red after importing the source-only
SEC transfer_001 review and probing legal-name/exhibit-treatment contract
guidance. The review rejected all ten inspected candidate variants. A safe
typed source-scope filing-id guard removed fixture-id-as-filing-id support, and
the contract probe recovered expected registrant/exhibit rows, while
source-reviewed `svc_now_inc` aliases remain as supported forbidden emissions
at support>=1:

```text
overall support>=2: 111/141
SEC seed/transfer support>=2: 13/13, 9/13, 10/12, 10/13
...
forbidden emissions support>=1/support>=2: 2/0
status: fail
```
