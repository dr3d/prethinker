...
| --- | --- | --- | --- |
| SEC Form 8-K | Skeleton only: filing wrapper, registrant, identifiers, item headings, item treatment, exhibits, signature block | Historical pre-axis-repair cells: seed `13/13`; three unlike transfers `13/13`, `12/12`, `12/12`; `0` supported forbidden. Repaired axis-clean breadth previously included transfer_001 `11/13`, but the current-mainline transfer_001 full-bundle retest first demoted the standing cell to `5/13`, and the imported source-only review `sec_t001_source_review_20260605` rejected ten candidate variants. The typed source-scope filing-id guard removes wrong-key fixture-id-as-filing-id rows, and legal-name/exhibit-treatment contract guidance lifts current standing transfer_001 to `9/13`; the cell remains red with `2/0` forbidden emissions at support>=1/support>=2 from source-reviewed `svc_now_inc` alias rows. Transfer_002 and transfer_003 also drop under the typed source-scope filing-id guard to `10/12` and `10/13`. The current-mainline seed retest remains `13/13`, `0/7` supported forbidden, but records 5 singleton unexpected item-treatment rows. Dense compile substitutions on transfer_003 both landed at `10/12`, `0` forbidden, clean gates. | Useful skeleton-transfer and boundary evidence under the local-Qwen-MoE lane, not a pristine or model-independent methods anchor. The repaired schema exposes transfer_001 source-reviewed registrant-alias emission, favorable-draw risk, wrapper/telephone recall gaps, duplicate file-number instability, item-treatment recall, MoE jitter, and model-path role-semantics sensitivity. The current standing manifest is red until the alias blocker closes without a fixture-specific reducer. |
...
governance but lower support than the historical pre-axis-repair claims. The
latest standing manifest is red after source-only transfer_001 review: a safe
typed source-scope guard removed wrong-key fixture-id support, but registrant
alias variants still emit at support>=1 and are not safe to repair with a
company-specific normalizer.
