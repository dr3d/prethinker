...
How Prethinker treats English as source code and compiles it into a queryable
logic program.
...
Prethinker starts somewhere softer and more dangerous: ordinary English.
...
Prethinker's job is to turn that English into a small logic program that can
answer those questions later. The target language is Prolog-like: a world made
...
That is the core mental shift. Prethinker is not "asking an LLM to answer from a
document." It is asking an LLM to help compile a document into a durable,
inspectable program. The answer should come from the compiled program, not from
the model's memory or mood.
...
### 2. Propose A Predicate Palette

Before writing facts, the compiler needs a vocabulary.
...
The compiler tries to admit facts that are:
...
```text
English source -> semantic reading -> candidate operations -> admission checks -> Prolog-like KB
```
...
```text
query-executable knowledge program
```
