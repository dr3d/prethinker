...
- Needed now: `False`
- Reason: The first known hallucination/sanction package was returned as legal_authority_known_hallucination_sanction_20260606_01.zip and retained outside active governance after failing intake validation. It is useful diagnostic material: after local parser hardening it extracts all 18 citation mentions with invalid_reporter=0 and false_verified=0, but it is still not ready to add to this manifest because its oracle facts use placeholder variables/source labels rather than concrete verifier fact rows. No new external package is needed until we either normalize the existing review locally or prepare a corrected concrete-fact work-order request. Any future corrected work-order request should be emitted as a fresh zip in C:\prethinker\tmp, not in the archive.
