...
| clinic recall support | Split, audit ongoing | Device/serial rows plus generic manufacturer-liaison, verification-procedure, and failure-rate atoms are labeled `clean-helper` when acquired by the source-record ledger; named-clinic, cabinet, seal, and authority recognizers remain `candidate-helper`. |
