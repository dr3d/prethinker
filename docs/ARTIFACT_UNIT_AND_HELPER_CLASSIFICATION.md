...
| roster state support | Split, audit ongoing | Admitted-predicate joins for adult roles, supervision, memberships, and group counts are labeled `clean-helper`; source-record roster parsing over school-roster shapes remains `candidate-helper`. The parser now covers both `v1/v2/v3` + `group_a/group_b/group_c` + `s_###` rows and sibling `v1_0/v1_3` + `7_a` + `STU-####` homeroom rows, but promotion waits for sibling QA replay evidence. |
