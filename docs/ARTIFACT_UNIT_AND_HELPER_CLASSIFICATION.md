...
| roster state support | Split, audit ongoing | Admitted-predicate joins for adult roles, supervision, memberships, and group counts are labeled `clean-helper`; source-record roster parsing over school-roster shapes remains `candidate-helper`. A first deterministic roster-table ledger emits `roster_table_member/4` plus printed-label facts for explicit table rows with both grouping and member columns, which lets that narrow surface be consumed as clean structural memory. Section/prose-derived roster membership remains candidate-helper. |
