...
| clinic recall support | Split, audit ongoing | Device/serial rows plus generic manufacturer-liaison, verification-procedure, acronym-derived clinic abbreviation, and failure-rate atoms are labeled `clean-helper` when acquired by the source-record ledger; non-initialism abbreviations, cabinet, seal, and authority recognizers remain `candidate-helper`. |
