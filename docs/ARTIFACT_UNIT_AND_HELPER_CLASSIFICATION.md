...
| industrial sensor support | Split, audit ongoing | Field-driven event/timestamp/count, computed-duration, packet-id, data-loss, lab-sample logistics, and system clock-authority rows are labeled `clean-helper`; stated-duration and remaining exact prose recognizers are labeled `candidate-helper`. |
