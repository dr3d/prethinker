...
| clinic recall support | Split, audit ongoing | Device/serial rows plus generic manufacturer-liaison, verification-procedure, explicit clinic abbreviations, cabinet/seal range, failure-rate atoms, and visit-date ranges are labeled `clean-helper` when acquired by the source-record ledger; key-retainer identity and named medical-director authority remain `candidate-helper` until blockquoted memo sender lines become addressable. |
