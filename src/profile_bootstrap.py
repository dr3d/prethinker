...
PROFILE_BOOTSTRAP_GUIDANCE = (
    ...
    "permissions/authorizations, requirements, exceptions, overrides, claims/evidence priority, and derived statuses. "
    "If the domain likely needs executable rule admission, include predicate families that can appear safely in rule "
    "heads and bodies; a profile with only rule_text labels is too weak for later Prolog inference.\n"
    "- For dense rule-source profile bootstrapping, stay inside a compact budget. Use fewer reusable predicate "
    "families rather than long exhaustive examples, and keep notes short. Complete rule extraction belongs to later "
    "semantic_ir passes; profile_bootstrap_v1 should only provide the vocabulary and admission-risk surface.\n"
    "- For insurance, reinsurance, contract-coverage, maritime-loss, warranty, salvage, or claims-adjustment sources, "
    "do not collapse the domain into generic claim_amount and policy_clause rows. Preserve separate row families for "
    "insured objects or assets, scoped party roles, contracts/policies/treaties, policy periods, limits, deductibles, "
    "underwriter or insurer shares, reinsurance attachment/limit layers, incident events, notifications and deadlines, "
    "cover suspension or reinstatement windows, expert/survey findings, source-attributed measurements, disputed cost "
    "positions, calculation steps, legal or coverage positions, cited authority, regulatory positions, corrections, "
    "security/guarantee versus payment status, and witness statements when the source supports those jobs. Later QA "
    ...
    "If the source gives vessel or asset attributes, propose a queryable attribute predicate such as asset_attribute/3, "
    "vessel_attribute/3, insured_value/3, vessel_owner/2, or vessel_manager/2. If the source gives insurer shares, "
    "propose a direct share predicate such as underwriter_line/4, insurer_share/4, or party_share/4 rather than relying "
    "on derived exposure text.\n"
    "- For financial-arithmetic profiles, preserve both raw amounts and calculation relationships. Candidate predicates "
    ...
    "query-bearing identity surfaces. If the source gives vendor, manufacturer, model, serial number, firmware, "
    "capacity, location, calibration scope, certification scope, or comparable descriptive attributes, propose "
    "compact reusable attribute predicates such as equipment_attribute/3, device_attribute/3, item_attribute/3, "
    "asset_attribute/3, or domain-owned equivalents like sensor_vendor/2 and sensor_model/2. A profile that keeps "
    "only the device id plus status/certification rows is shallow when the source states distinct specification "
    "fields.\n"
    "- For role-bearing source lines, signatures, headers, attendance notes, correspondence blocks, source-authored notes, "
    "or similar source-stated person-plus-role text, propose a direct "
    ...
    "- For temporal-distance language, printed relative intervals are first-class query surfaces. If the source says "
    ...
    "- Keep argument role names short and categorical, such as source, subject, value, unit, status, basis, date, "
    "amount, party, role, contract, start_time, and end_time. Do not put example values, long alternatives, "
    "or enumerated possibilities inside args. Values belong in later candidate_operations, not in the profile schema.\n"
    ...
    "- Explicit negative surfaces need their own queryable predicate when the source treats them as facts. "
    "If the source says an actor lacks authority, has no veto/control, is not allowed, is exempt, or is outside scope, "
    "prefer a positive predicate for the stated negative relation such as lacks_authority/2, lacks_veto/2, "
    "prohibition/3, is_not_allowed_to_approve/2, exemption/3, or outside_scope/2. Do not rely on a positive predicate "
    "like has_veto/2 plus an admission note that negation is significant; the later compiler cannot safely assert a "
    "general negative fact from that alone.\n"
...
PROFILE_BOOTSTRAP_REVIEW_GUIDANCE = (
    ...
    "- Flag negative-surface loss: if raw source text directly states a lack of authority, no-veto/no-control relation, "
    "not-allowed role, exemption, or outside-scope condition, but the proposed profile only offers the corresponding "
    "positive predicate plus a note that negation matters, recommend an explicit negative-relation predicate instead.\n"
    "- Flag temporal-distance loss: if raw source text prints a relative interval such as three years before or ten "
    "days after, but the proposed profile only has before/after or published-before predicates without a distance "
    "slot, recommend a duration/proximity predicate rather than a boolean-only temporal surface.\n"
    ...
    "- Flag insurance-dispute under-surfacing: if the source contains policies, insurer shares, deductibles, "
    "reinsurance layers, expert reports, disputed amounts, notifications, coverage defenses, salvage security, "
    "or regulatory positions and the proposed profile cannot query those as separate row classes, recommend a retry "
    "with contract/policy roles, amount/calculation rows, source-attributed expert findings, temporal notice rows, "
    ...
    "or asset rows with explicit vendor, manufacturer, model, serial, capacity, firmware, location, calibration, "
    "certification, or comparable specification fields, and the proposed profile only has id/status/certification "
    "predicates that cannot recover those stated fields separately, recommend a reusable attribute capability such as "
    "equipment_attribute/3, device_attribute/3, item_attribute/3, asset_attribute/3, or precise domain equivalents "
    "like sensor_vendor/2 and sensor_model/2.\n"
    "- Flag source-role loss: if the source contains a signature, header, attendance note, correspondence block, "
    "source-authored note, office/title label, or similar person-plus-role line, and the proposed "
