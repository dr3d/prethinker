...
SCHEMA_CONTRACT: dict[str, Any] = {
    ...
    "query_intents": [
        {
            "intent_type": "list|count|date|source_location|heading_scope|note_marker|contact|signatory|comparison|duration|status|ordered_labeled_entry|amount_inventory|ratio_calculation|duration_quantity|biography_history|employment_history|role_transition|board_nominee_path|named_role_roster|document_chronology|unknown",
            "target_terms": [],
            "answer_constraints": [],
...
BEST_GUARDED_V2_GUIDANCE = (
    ...
    "- Normalize entity atoms only for spelling/case/spacing/plural cleanup, explicit aliases in the utterance/context/domain ontology, or KB-resolved identity. If the utterance uses a distinctive local name, do not replace it with a famous-story or archetype alias unless the utterance/context explicitly supplies that alias. Keep the text-local name and list any tempting prior alias in self_check.notes or unsafe_implications instead of committing it.\n"
    "- Preserve distinctive text-local names when forming atoms. Do not shorten a multi-word local name to a generic type when the source phrase contains identity-bearing words, unless an explicit alias is supplied.\n"
    ...
    "- Preserve stable object identity across segments. Reuse object atoms already present in context for the same object. For possessive story objects, prefer possessor-scoped atoms over generic size-only atoms when the possessor is source-stated. If size is mentioned, it may inform the object atom only when no possessor is available.\n"
    "- If a segment says an observer saw that an unknown person had acted on an object, commit the passive object state when available, not a vague saw(observer, event_something_...) write. Prefer was_tasted(possessor_scoped_food) and omit the unknown actor/event wrapper unless a precise observed-event predicate is available.\n"
    ...
    "- For query turns, populate query_intents[] when the question asks for a particular answer shape, source location, note marker, ordered list, amount inventory, ratio calculation, date, count, status, duration, duration quantity, biography/employment history, comparison, contact/reply/contact-email/attention-line information, signatory, role transition, board-nominee path, named-role roster, or document chronology. query_intents is proposal metadata for deterministic query routing; it is not durable truth and does not authorize writes.\n"
