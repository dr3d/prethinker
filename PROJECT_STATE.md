...
- Second action from the planner: a combined object/state/custody compile over
  ...
  compile prompt.
- Third action from the planner: the temporal/status/deadline slice split into
  an Ashgrove selector win and a Copperfall transfer rejection. Ashgrove's
  broad temporal compile scored only `19 / 5 / 1`, below the operational
  variant at `21 / 4 / 0`, but adding it to the frozen selector roster exposed
  a `24 / 1 / 0` upper bound. Reason-named adjusted-expiration and
  correction-entitlement guards reach that bound with `25/25` selected-best
  rows and `0` selector errors. Copperfall's broad temporal compile regressed
...
- `scripts/run_semantic_ir_stress_sweep.py` now provides a broad mixed-domain stress harness. It balances scenario sources, generates typo/bad-grammar/context-switch variants, can repeat temperature-0 runs for signature variance checks, applies admitted writes to a private stream KB, and records admitted queries without executing them by default so recursive toy-Prolog queries cannot monopolize CPU.
...
- Keep measuring router/compiler/admission quality directly on Glitch, Ledger, medical ambiguity, temporal packs, and current fixture batteries. Archived mixed-domain stress packs can still serve regression-calibration questions, but they are not the active frontier.
...
- Selector-only mixed-domain stress smoke after cleanup: `15/15` cases traversed, `12/12` checked selectors, `0` fuzzy edge kinds, with all remaining `kb_scenarios/*.json` fixtures mined by the stress loader.
...
- Historical fast mixed-domain stress smoke: `15/15` parsed JSON, `11/11` selector-checked, `5/15` records with scoped epistemic memory, and `{}` fuzzy edge kinds in about 3.5 minutes on the local 35B LM Studio path.
- Soup-to-nuts shakedown after the cleanup exposed a practical testing boundary: broad `scenario-group=all` and multi-variant stress runs are useful historical calibration jobs, but too slow and too indirect for frequent active research checks.
...
- Historical mixed-domain v5 source-record alignment rerun: `60/60` parsed JSON, `60/60` domain selector, `60/60` admission-safe, `45/60` semantic-clean, `41/60` full expectation score, `0/60` temp-0 signature variance groups, and `{}` fuzzy edge kinds. Keep this as calibration history: remaining misses were mainly missing semantic anchors, temporal label calibration, and exception-rule representation rather than unsafe durable writes.
