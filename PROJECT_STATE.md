...
- Raw-file bootstrap smoke on `tmp/declaration.md` with `qwen/qwen3.6-35b-a3b`: parsed `profile_bootstrap_v1`, rough score `1.000`, `10` candidate predicates, `0` generic predicates, and `0` unknown positive predicate refs in generated starter cases. Optional whole-source compile failed at Semantic IR parse/output length, which usefully marks the next missing mechanism: LLM-owned intake planning/segmentation rather than Python text splitting.
