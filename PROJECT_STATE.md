...
- Raw Proclamation profile-review experiment without target-signature handholding: useful run produced `30` candidate predicates, `93` admitted source-compile operations, `4/4` successful planned passes, and first-20 post-ingestion QA at `10 exact / 5 partial / 4 miss / 1 not judged`. After adding explicit source-record/reporter/condition pressure, a later product-like compile produced a stronger profile shape (`18` predicates, `4` repeated structures, `497` admitted operations, profile rough score `1.000`). After structural query-placeholder normalization, the first-20 QA pass reached `12 exact / 2 partial / 6 miss`. The remaining frontier is no longer raw ingestion alone; it is stabilizing model-owned predicate surfaces and compiled-KB query planning.
