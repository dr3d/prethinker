...
**Current headline:** after retiring the legacy parser-lane tests, unreachable JSON fixtures, the old clarification side surface, and raw-utterance family/social rewrite crutches, the lean full pytest suite is `354 passed`. The current fast selector-only Lava smoke covers `15/15` mixed cases with `12/12` checked selectors and `0` fuzzy edge kinds. Current best demo pressure point remains the reimbursement policy cross-turn run: `4/4` parsed, `4/4` apply-error-free, `4/4` expected query matches, and no derived violation write leak.
...
- Blackthorn hostile procedural-misconduct fixture admission: full suite `354 passed`; fixture contains source story, gold KB, 100 QA probes, ontology registry, support map, failure buckets, and progress journal. Baseline first-20 QA was `2 exact / 1 partial / 17 miss`. BTC-015 proved compact `source_pass_ops_v1` focused passes could make registry-visible planning useful (`105` admitted, first-20 `11 / 3 / 6`). BTC-016 remains the compile high-water: source-boundary guidance fixed `source=context` proposals without weakening the mapper, reaching `190` admitted operations, only `3` skipped operations, expected-signature recall `0.719`, first-20 QA `15 exact / 2 partial / 3 miss`, loose support `20/20`, and loose query-signature matches `35/37`. BTC-021 is the current full-100 QA high-water on the same compiled KB: narrow deadline/duration query-planning guidance plus deterministic temporal subset fallback reached `75 exact / 4 partial / 21 miss`, with `100/100` parsed workspaces, `100/100` query rows, `0` runtime load errors, and `0` write proposals. Later branches showed three negative lessons: widening focused passes improved support shape without improving QA, changing argument contracts midstream is a profile migration rather than a tuning knob, and broad procedural-finality guidance can rescue one appealability question while regressing stable query plans.
...
- Proposition-spine smoke: strict LM Studio structured output accepted the v1-compatible schema extension. The model emitted `propositions[]` and linked all candidate operations by `proposition_id`; mapper diagnostics carried the proposition data while admitting truth through the existing operation gates. Full suite: `343 passed`.
- Full suite after retiring unreachable JSON fixtures and parser-era family/social rewrite crutches: `339 passed`.
