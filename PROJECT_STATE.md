...
- Raw Proclamation profile-review experiment without target-signature handholding: useful run produced `30` candidate predicates, `93` admitted source-compile operations, `4/4` successful planned passes, and first-20 post-ingestion QA at `10 exact / 5 partial / 4 miss / 1 not judged`. The remaining frontier is stabilizing model-owned predicate surfaces and query planning.
