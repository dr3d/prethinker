...
**Current headline:** latest full pytest suite `394 passed`; latest Lava v5 sampled run stayed `60/60` parsed JSON, `60/60` domain selector, `60/60` admission-safe, `45/60` semantic-clean, `41/60` full expectation score, `0` fuzzy edge kinds, and `0/60` temp-0 signature variance groups. Current best demo pressure point remains the reimbursement policy cross-turn run: `4/4` parsed, `4/4` apply-error-free, `4/4` expected query matches, and no derived violation write leak.
...
- Full pytest after post-ingestion QA judging, compiled-query templates, and structural query-placeholder hygiene: `390 passed`.
- Full pytest after profile-bootstrap review/retry control-plane work and expanded structural query-placeholder hygiene: `392 passed`.
- Full pytest after profile-owned predicate alias canonicalization: `394 passed`.
- Raw Proclamation profile-review experiment without target-signature handholding: the useful run produced `30` candidate predicates, `93` admitted source-compile operations, `4/4` successful planned passes, and first-20 post-ingestion QA at `10 exact / 5 partial / 4 miss / 1 not judged`. The reviewer recovered queryable source-detail predicates such as `ledger_conflict/4`, `ledger_entry/3`, and `grievance_reporter/2`, but follow-up reruns exposed temperature-0 predicate-surface variance and query-planning sensitivity. The next hard frontier is stabilizing model-owned predicate surfaces and query planning, not adding Python NLP.
