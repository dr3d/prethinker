...
- Python compile check for touched runtime files passed; full pytest suite now `314 passed`
- Focused truth-maintenance schema/trace verification: `54 passed` across semantic IR runtime and trace rendering; compile check passed for touched Semantic IR scripts. Live LM Studio `qwen/qwen3.6-35b-a3b` truth-maintenance smoke covered correction-plus-claim, rule-consequence-as-query, and observed-document-vs-witness-claim turns. The model emitted support links, conflicts, retraction plans, and `query_only`/`do_not_commit` consequences while the mapper admitted only candidate-operation effects.
