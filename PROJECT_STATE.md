...
- Keep measuring router/compiler/admission quality directly on Glitch, Ledger, medical ambiguity, temporal packs, and current fixture batteries. Archived mixed-domain Lava packs can still serve regression-calibration questions, but they are not the active frontier.
...
- Historical fast Lava smoke: `15/15` parsed JSON, `11/11` selector-checked, `5/15` records with scoped epistemic memory, and `{}` fuzzy edge kinds in about 3.5 minutes on the local 35B LM Studio path.
- Soup-to-nuts shakedown after the cleanup exposed a practical testing boundary: broad `scenario-group=all` and multi-variant Lava runs are useful historical calibration jobs, but too slow and too indirect for frequent active research checks.
